...
export async function GET(request: Request) {
  try {
    ...
    const entries = await prisma.manufacturingProductionEntry.findMany({
      where,
      include: { item: true },
      orderBy: { date: 'desc' }
    })
...
export async function POST(request: Request) {
  try {
    ...
    const { itemId, date, quantity, unit, remarks, createdBy } = body
    ...
    const entry = await prisma.$transaction(async (tx) => {
      ...
      const created = await tx.manufacturingProductionEntry.create({
        data: {
          itemId,
          date: entryDate,
          quantity: qty,
    ...
    const message = error instanceof Error ? error.message : 'Failed to create production entry'
